...
- 单次发送最多 4 张图片
- 超过 4 张时，前端直接阻止继续添加并给出提示
- 后端对单次消息的 `attachment_ids` 数量做兜底校验，避免前端绕过
...
- `attachment_ids.length <= 4`
- 超出时拒绝创建 turn，并返回明确错误信息
...
2. 拖拽图片后出现预览
3. 删除待发送图片后预览消失
4. 添加第 5 张图片时前端给出上限提示
5. 发送带图片消息时调用上传接口并传出 `attachmentIds`
6. 已发送消息图片可正常渲染
...
2. 非图片上传被拒绝
3. `attachment_ids` 超过 4 个时拒绝创建消息
4. 带 `attachment_ids` 的消息能生成附件元数据
5. `ContextAssembler` 能生成图片 content parts
6. `supports_vision == False` 时不注入图片
7. `edit_and_rerun` 继承原附件
...
1. 修图片展示 URL 与回显稳定性
2. 在前后端同时落地“单次最多 4 张图片”
3. 统一 `supports_vision`
4. 补孤儿附件清理
5. 修编辑重跑附件继承
6. 补齐前后端和端到端测试
