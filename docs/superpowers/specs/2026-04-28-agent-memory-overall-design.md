...
第一阶段里，真正实用的“工作记忆主干”就是 conversation 对应的事实表。
...
这些最有价值的信息，几乎都应该从 conversation 事实层拿，而不是额外存成另一套 memory。
...
这是当前 session 的最近精确历史窗口，直接来自 conversation 事实层。
...
conversation 事实层是底座，不是“记忆对象”；recall 是 conversation 的读取能力，不是独立存储对象。
...
第一阶段优先不新建重型 memory 表，而是直接复用现有 conversation 体系。
...
1. conversation snapshot + events
2. 项目级 `USER.md / MEMORY.md`
3. 最近精确 transcript window
...
2. 基于现有 conversation 表的 recent exact window
3. 由文本压缩直接生成的 continuation artifact
4. 基于 conversation 表的 transcript recall
...
- `messages + conversation_events` 已经是 recall 与 continuation 的天然语料
