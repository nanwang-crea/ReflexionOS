...
class CodexPatchParser:
    ...
    def parse(self, patch_text: str) -> CodexPatch:
        ...
        if not lines or lines[0] != "*** Begin Patch":
            raise ValidationError(message="Codex-style patch 必须以 *** Begin Patch 开始")
        if lines[-1] != "*** End Patch":
            raise ValidationError(message="Codex-style patch 必须以 *** End Patch 结束")

        operation_lines = [line for line in lines if line.startswith(self.OPERATION_PREFIXES)]
        if not operation_lines:
            raise ValidationError(
                message="Codex-style patch 缺少文件操作头，"
                "请使用 *** Add File、*** Update File 或 *** Delete File"
            )
        if len(operation_lines) > 1:
            raise ValidationError(message="Codex-style patch 仅支持单文件修改")
    ...
    def _parse_add_file(self, file_path: str, body: list[str]) -> CodexPatch:
        ...
        for line in body:
            ...
            if not line.startswith("+"):
                raise ValidationError(message="Codex-style Add File 内容行必须以 + 开头")
    ...
    def _parse_update_file(self, file_path: str, body: list[str]) -> CodexPatch:
        ...
        for line in body:
            if line.startswith("*** Move to: "):
                raise ValidationError(message="Codex-style Move/Rename 暂不支持，请拆成新增和删除")
            if line == "*** End of File":
                continue
            if line.startswith("@@"):
            ...
            if current_hunk is None:
                raise ValidationError(message="Codex-style Update File 缺少 @@ hunk header")
            if not line.startswith(("+", "-", " ")):
                raise ValidationError(message="Codex-style hunk 行必须以空格、+ 或 - 开头")
            current_hunk.append(line)

        if current_hunk is not None:
            hunks.append(current_hunk)
        if not hunks:
            raise ValidationError(message="Codex-style Update File 未找到有效 hunk")
    ...
    def _parse_delete_file(self, file_path: str, body: list[str]) -> CodexPatch:
        ...
        for line in body:
            if line == "*** End of File":
                continue
            raise ValidationError(message="Codex-style Delete File 不应包含 hunk 内容")
