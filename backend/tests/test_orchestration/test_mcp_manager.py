...
class TestMCPManager:
    ...
    async def test_register_server(self):
        manager = MCPManager()
        config = MCPServerConfig(server_id="test_server", command="node", args=["server.js"])
...
class TestMCPServerConfig:
    def test_config_creation(self):
        config = MCPServerConfig(
            server_id="test", command="node", args=["server.js"], env={"NODE_ENV": "production"}
        )
    ...
    def test_config_default_values(self):
        config = MCPServerConfig(server_id="minimal", command="test")
...
class TestMCPTool:
    def test_tool_creation(self):
        tool = MCPTool(name="test_tool", description="测试工具", input_schema={"type": "object"})
