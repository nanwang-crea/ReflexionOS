...
@pytest.mark.asyncio
async def test_recover_orphaned_approvals_cancels_run_left_waiting_across_restart(
    monkeypatch, tmp_path
):
    """模拟"审批中途系统退出、重进会话"场景：
    上一进程留下一条 WAITING_FOR_APPROVAL 的 Run（DB 落库），但审批相关的内存态
    （PendingApprovalStore/执行循环）已随进程销毁。新进程启动时调用
    recover_orphaned_approvals 应该能在不依赖任何内存态的情况下，把该 Run 清理为
    CANCELLED，避免前端展示一张永远无法响应的审批卡片。
    """
    project = Project(id="project-1", name="ReflexionOS", path=str(tmp_path))
    session = Session(id="session-1", project_id="project-1", title="需求讨论")
    provider = build_provider("provider-a", "Provider A", ["model-a"])
    ...
    # 第一个 service 实例模拟"上一次进程"：写入等待审批状态后即视为进程退出（不做任何清理）。
    old_service, conversation_service, dummy_config = build_service_with_db(
        monkeypatch,
        tmp_path,
        project=project,
        session=session,
    ...
    old_service.pending_approval_store.create(
        approval_id="approval-1",
        session_id="session-1",
        turn_id=started.turn.id,
        run_id=started.run.id,
    ...
    run_before_restart = conversation_service.get_run(started.run.id)
    assert run_before_restart.status == RunStatus.WAITING_FOR_APPROVAL

    # 新建一个全新的 AgentService，模拟进程重启：PendingApprovalStore 是空字典，
    # _execution_loops/running_tasks 等内存态也都是空的，只有 DB 里的 Run 记录还在。
    new_service = agent_service_module.AgentService(
        project_repo=ProjectRepository(Database(str(tmp_path / "agent-service.db"))),
        session_repo=SessionRepository(Database(str(tmp_path / "agent-service.db"))),
        conversation_service=conversation_service,
        llm_provider_service=LLMProviderService(config_manager=dummy_config),
    )
    assert new_service.pending_approval_store.get("approval-1") is None

    recovered_count = await new_service.recover_orphaned_approvals()

    assert recovered_count == 1
    recovered_run = conversation_service.get_run(started.run.id)
    assert recovered_run.status == RunStatus.CANCELLED
    # 二次调用应幂等：不应再有孤儿 Run 可清理
    assert await new_service.recover_orphaned_approvals() == 0
