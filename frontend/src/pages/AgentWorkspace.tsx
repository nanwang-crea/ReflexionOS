...
export default function AgentWorkspace() {
  ...
  const { messages, isRunning, plan, hasMore, oldestLoadedTurnId } =
    useConversationData(currentSessionId)
  const agentMode: AgentMode = useConversationStore((s) =>
    currentSessionId
      ? s.agentModeBySessionId[currentSessionId] ?? 'build'
      : 'build'
  )
  const runsById = useConversationStore((s) =>
    currentSessionId
      ? s.conversationsBySessionId[currentSessionId]?.runsById
      : undefined
  )
  ...
  const handleDetailClick = useCallback(
    (detail: ActionReceiptDetail) => {
      ...
      if (!path) return
      const viewMode = ['edit', 'create', 'delete'].includes(detail.category)
        ? 'diff'
        : 'edit'
  ...
  const viewModel = useCurrentSessionViewModel({
    ...
    plan,
    hasMore,
    onLoadMore: currentSessionId
      ? (beforeTurnId) => loadMore(currentSessionId, beforeTurnId)
      : undefined,
  ...
  return (
    <>
      <div className="flex h-full">
        <div
          className={`flex h-full flex-1 min-w-0 flex-col bg-surface-primary ${
            workspaceTab === 'code' ? '' : 'hidden'
          }`}
        >
        ...
        >
          ...
          <div className="border-t border-edge bg-surface-primary">
            <div
              data-chat-input-frame
              className="mx-auto w-full max-w-[1280px] p-4"
            >
